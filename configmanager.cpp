...
template<typename T> T ConfigManager::getValue(const QStringList& keys,const T& defaultValue){
    QJsonObject obj=m_root;
    for(int i=0;i<keys.size();i++){
        if(!obj.contains(keys[i])||obj[keys[i]].isObject()) return defaultValue;
        obj=obj[keys[i]].toObject();
    }
    QString lastKey=keys.last();
    if(!obj.contains(lastKey)) return defaultValue;
    QJsonValue val=obj[lastKey];
    if constexpr (std::is_same_v<T,qreal>)return val.toDouble(defaultValue);
    else if constexpr (std::is_same_v<T,QString>)return val.toString(defaultValue);
